...
public class Calcul {
	...
	public boolean estOrienteVersPion (PionOriente pion1, PionOriente pion2) {
		...
		if ((pion1.getOriente() == 'h') && (pion1.getPositionY() > pion2.getPositionY())) {
			oriente = true;
		}
		if ((pion1.getOriente() == 'b') && (pion1.getPositionY() < pion2.getPositionY())) {
			oriente = true;
		}
		if ((pion1.getOriente() == 'g') && (pion1.getPositionX() > pion2.getPositionX())) {
			oriente = true;
		}
		if ((pion1.getOriente() == 'd') && (pion1.getPositionX() < pion2.getPositionX())) {
			oriente = true;
		}
		return oriente;
	}
	...
	public boolean ProcheLignesouColonnes (Robot player1, Robot player2) {
		...
		if (Math.abs(player1.getPositionX()-player2.getPositionX()) > Math.abs(player1.getPositionY()-player2.getPositionY())) {
			return true; 		// proche des lignes
		} else if (Math.abs(player1.getPositionX()-player2.getPositionX()) == Math.abs(player1.getPositionY()-player2.getPositionY())) {
	...
	public void OrienteVersLignes (Robot player1, Robot player2) {
		...
		case 'd' : 	player1.ChangerOrientationRobot(player1, 1);
					break;
		case 'h' :	if (player1.getPositionY() > player2.getPositionY()) {
						player1.DeplacementAV();
					} else {
						player1.DeplacementAR();
					}
					break;
		case 'b' : 	if (player1.getPositionY() > player2.getPositionY()) {
						player1.DeplacementAR();
					} else {
						player1.DeplacementAV();
					}
	...
	public void OrienteVersColonnes (Robot player1, Robot player2) {
		...
		case 'b' : 	player1.ChangerOrientationRobot(player1, 1);
					break;
		case 'g' :	if (player1.getPositionX() > player2.getPositionX()) {
						player1.DeplacementAV();
					} else {
						player1.DeplacementAR();
					}
					break;
		case 'd' : 	if (player1.getPositionX() > player2.getPositionX()) {
						player1.DeplacementAR();
					} else {
						player1.DeplacementAV();
					}
